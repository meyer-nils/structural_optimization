...
\begin{enumerate}
    ...
    \item Repeat steps 2, 3, and 4 until you reached a maximum number of iterations $k \le N$ or there is no improvement anymore.
\end{enumerate}
...
\begin{itemize}[label=$\dots$]
    ...
    \item discuss the relation between these approximation methods and choose an appropriate method for a given task
\end{itemize}
\end{objectives}
...
\begin{align}
    \tilde{g}(\mathbf{x}) &= g(\mathbf{x}^k) + \nabla g(\mathbf{x}^k) \cdot \left(\mathbf{x} - \mathbf{x}^k \right) \\
    \tilde{h}(\mathbf{x}) &= h(\mathbf{x}^k) + \nabla h(\mathbf{x}^k) \cdot \left(\mathbf{x} - \mathbf{x}^k \right).
\end{align}
