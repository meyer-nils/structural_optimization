...
\begin{equation}
    \hat{\rho}_j^{k+1} = \left(G_j^k\right)^\alpha \rho_j^k
\end{equation}
drives the solution towards the optimal point $\rho_j^*$, where $G^*_j=1$. The exponent $\alpha \in (0,1]$ stabilizes the algorithm with typical values being $\alpha=0.5$ \cite{Harzheim2014}.
...
\begin{example}{Algorithm with optimality criteria}{ocexample}
    ...
    \begin{enumerate}
        ...
        with
        \begin{equation}
            \hat{\rho}_j^{k+1} = \left(\frac{ - \widetilde{\frac{\partial C}{\partial \rho_j}}}{\mu V_j}\right)^\alpha \rho_j^k.
        \end{equation}
...
\begin{example}{Shape optimization of a cantilever beam}{cantilevershapeoptimizationexample}
    ...
    \begin{enumerate}
        ...
        \item Perform a line search to find the maximum in 
            ...
            \end{equation}
            with a gradient descent method. Once again, we may use automatic differentiation to compute the gradient for the optimization.
