...
\begin{example}{Three bar truss shape optimization}{trussshapeoptimizationexample}
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            L^k_i = 
            \begin{cases}
                x^k_i - s  (x^{k-1}_i-L^{k-1}_i) & (x_i^k-a_i^{k-1})(x_i^{k-1}-x_i^{k-2}) < 0\\
                x^k_i - \frac{1}{\sqrt{s}}  (a^{k-1}_i-L^{k-1}_i) & \text{else}
            \end{cases}
        \end{equation}
        ...
        \begin{equation}
            U^k_i = 
            \begin{cases}
                x^k_i - s  (U^{k-1}_i-x^{k-1}_i) & (x_i^k-a_i^{k-1})(x_i^{k-1}-x_i^{k-2}) < 0\\
                x^k_i - \frac{1}{\sqrt{s}}  (U^{k-1}_i-a^{k-1}_i) & \text{else}
            \end{cases}
        \end{equation}
