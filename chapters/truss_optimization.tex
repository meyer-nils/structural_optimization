...
\begin{example}{Fully stressed design}{mitchellexample}
    ...
    \begin{equation}
        \frac{\partial \mathcal{L} (\mathbf{a}, \mu)}{\partial a_j} = - \frac{\sigma^2_j}{E} l_j + \mu l_j
    \end{equation}
...
\end{equation}
for $\mu^*>0$. This solution procedure is simple - the dual Lagrangian depends only on one variable and the problem is concave. We can use a gradient descent method to solve it quickly and utilize this analytical expression for the gradient 
\begin{equation}
